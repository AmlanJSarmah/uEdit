...
{ 
    if(tcgetattr(STDIN_FILENO, original_terminal_config) == -1) emergency_exit("tcsetattr");
    ...
    if(tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw_config) == -1) emergency_exit("tcsetattr");
}

void disable_raw_mode(struct termios *original_terminal_config)
{
    if(tcsetattr(STDIN_FILENO, TCSAFLUSH, original_terminal_config) == -1) emergency_exit("tcsetattr");
}
